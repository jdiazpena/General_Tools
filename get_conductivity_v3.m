function [ data, input ] = get_conductivity_v3( alt, Tein, electronDensity,...
    latitude, longitude, time, mode, temode, outputs, setPlotConductivity,...
    iriData, msisData)
...
% Add H and He in ions and neutrals
           %   O       N2      O2      H      He   
    C_in = [   0   ,   6.84,   6.64,   0,     1.32;   % OI
               2.44,   4.34,   4.27,   0.69,  0.74;   % NOI
               2.31,   4.13,   0,      0.65,  0.7 ;   % O2I
               4.42,   7.47,   7.25,   1.45,  1.49;   % NI
               0   ,   33.6,   32.0,   0,     4.71;   % HI
               10.1,   16.0    15.3    4.71   0  ];   % HeI
...
% Necessary Constants
    ...
    Z_i = [16; 30; 32; 14; 1; 4]; % Ion mass in proton mass units
    m_i = Z_i*m_p;
    q_i = [1;1;1;1;1;1]; % Ion charge
    ...
    for i=1:length(m_i)
        for n = 1:length(m_n)
            ...
            if C_in(i,n) == 0
                ...
                elseif i==1 && n==4
                    %OI-H
                    v_in(Tr>300,i,n) = (4.63*10^-12).*Nn((Tr>300),n).*((Tr(Tr>300)/8).^0.5);
                elseif i==5 && n==4
                    %HI-H
                    v_in(Tr>50,i,n) = (2.65*10^-10).*Nn((Tr>50),n).*(Tr(Tr>50).^0.5).*(1-0.083.*log10((Tr(Tr>50)))).^2;
                elseif i==6 && n==5
                    %HeI-He
                    v_in(Tr>50,i,n) = (8.73*10^-11).*Nn((Tr>50),n).*(Tr(Tr>50).^0.5).*(1-0.093.*log10((Tr(Tr>50)))).^2;
                end
            else
